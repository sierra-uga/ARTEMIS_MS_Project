...
inflow_metadata <- read_delim(
  "required_files/artemis-eDNA-metadata-final.tsv",  # read in inflow_metadata file
  delim = "\t",
  escape_double = FALSE,
  trim_ws = TRUE
)
# test to make sure they sample names are the same
#list1 <- inflow_metadata$sample_name
#list2 <- colnames(abundance_data)
#list2[!(list1 %in% list2)] # output: [1] "STN115.35.fil.dura.r1"

inflow_metadata <- inflow_metadata %>% filter(., Sample.Control == "True.Sample") %>% filter(., sample_name != "STN115.35.fil.dura.r1") %>% 
  filter(., sample_name != "STN089.200.fil.dura.r2") %>% filter(., Transect_Name == "transect1") #%>% distinct(Filter_pores, .keep_all = TRUE) 
...
dataframe_to_combine <- inflow_metadata %>% select(sample_name, Transect_Number, Station, Filter_pores) 
