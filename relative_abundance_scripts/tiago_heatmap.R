...
aldex2_ec_result <- KO_iron_abundance_type
...
ha_top_ec = HeatmapAnnotation(border = TRUE,
                              Experiment = as.vector(ec_sample_tab$Depth_Threshold),
                              Sample = as.vector(ec_sample_tab$Location),
                              col = list(
                                Experiment = c("Surface" = "#e0ecf4", "Intermediate" = "#9ebcda", "Bottom_water" = "#8856a7"),
                                Sample = c(c("Eastern_CC" = "#888", "Western_CC" = "dodgerblue", "Open_polynya" = "red", "Dotson" = "green"))
                              ),
                              annotation_legend_param = list(
                                ...
                                Sample = list(
                                  title = "Sample Type",
                                  at = c("Eastern_CC", "Western_CC", "Open_polynya", "Dotson"),
                                  labels = c("Eastern_CC", "Western_CC", "Open_polynya", "Dotson")
                                )
                              ))
...
aldex2_ec_result_name <- aldex2_ec_result_name[, -(1:249)]
...
head(ec_count_total)
ec_count_total <- ec_count_total[, -(1:249)]
head(ec_count_total)
...
process_col_discr = gpar(c("Carbon", "Nitrogen", "Sulfur"),
                         c("#af8dc3", "#f7f7f7", "#7fbf7b"))
colors = structure(1:8, names = unique(z_score_aldex2_ec_result_name_count_total$Metabolism))
...
ha_right_ec2 = rowAnnotation(
  Process = z_score_aldex2_ec_result_name_count_total$Metabolism, border = TRUE, col  = list(Process = colors))
row_labels_ec = z_score_aldex2_ec_result_name_count_total$Metabolism
...
hm_ec_sel <- Heatmap(aldex2_ec_result_czm_trans_trav_group, name = "Z-score, CLR", col = col_matrix3,
                     ...
                     column_split = as.vector(ec_sample_tab$Depth_Threshold),
                     border = TRUE,
