...
obj <- phyloseq_to_deseq2(ps_part, ~ Depth_Threshold + Iron_Level)
diagdds = DESeq(obj, test="Wald", fitType="mean", full= design(obj))

res = results(diagdds, cooksCutoff = FALSE)
alpha = 0.01
sigtab = res[which(res$padj < alpha), ]
sigtab = cbind(as(sigtab, "data.frame"), as(tax_table(ps_part)[rownames(sigtab), ], "matrix"))
head(sigtab)

#sigtab_new <- dplyr::inner_join(sigtab, iron_KO, by="KO_Num")

ggplot(sigtab, aes(x=Genus, y=log2FoldChange, color=Order)) +
  geom_point(size=3) + #scale_x_discrete(labels = new$Genus) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + ggtitle("Outflow (+) vs Inflow (-)")
...
KO_sig_numbers <- KO_sig_tab$KO_Num



ps_part_final <- subset_taxa(ps_free, unique %in% KO_sig_numbers)

colors = structure(1:14, names = unique(sigtab_new$Pathway.Final.Annotation.used.in.Fig.3.5))

colors2 = structure(1:14, names = unique(sigtab_new$Metabolism))
ha_right_ec2 = rowAnnotation(
  Pathway = KO_sig_tab$Pathway.Final.Annotation.used.in.Fig.3.5, border = TRUE, col  = list(Process = colors))

ha_right_ec1 = rowAnnotation(
  Nutrient = KO_sig_tab$Metabolism, border = TRUE, col  = list(Process = colors2))
...
htmp_part <- ps_part_final %>%
  ...
  comp_heatmap(
    ...
    sample_anno = sampleAnnotation(border = TRUE,
      "AOI" = anno_sample("Pos_in_polynya"),
  ...
  ) + ha_right_ec1 + ha_right_ec1 #+ row_labels_ec
