...
#ps_sub <- subset_samples(ps_sub, watertype != "Open_polynya") # if want to remove open polynya
ps_sub <- subset_samples(ps_sub, watertype != "Other")
# free-living
...
color_breaks <- c("AASW", "AASW-WW", "WW", "WW-CDW", "CDW")
color_point <- scale_fill_manual(values = c("#ffd412", "#FFAA33", "#FF55AA", "#AA55FF", "#800080"),
                                 name = "Water Mass",
                                 breaks = color_breaks,
                                 labels = c("AASW", "AASW-WW", "WW", "WW-CDW", "CDW"))

color_breaks <- c("AASW", "AASW-WW", "WW", "WW-CDW", "CDW")
color_point2 <- scale_color_manual(values = c("#ffd412", "#FFAA33", "#FF55AA", "#AA55FF", "#800080"),
                                   name = "Water Mass",
                                   breaks = color_breaks,
                                   labels = c("AASW", "AASW-WW", "WW", "WW-CDW", "CDW"))
...
free_above_CCA <- plot_ordination(ps_free_above, cca_free_above,
                                  type = "sites", color="watertype")
f_CCA <- free_above_CCA + geom_point(aes(fill = watertype), size = 3, alpha = 0.7) + ggtitle("Free-living") +remove_grid + color_point + color_point2 +#geom_label(size= 2, aes(label = Genus)) +  # X for other watertypes
...
free_below_CCA <- plot_ordination(ps_free_below, cca_free_below,
                                  type = "sites", color="watertype")
f_CCA2 <- free_below_CCA + geom_point(aes(fill = watertype), size=3, alpha=0.7) + remove_grid + color_point + color_point2+#geom_label(size= 2, aes(label = Genus)) +  # X for other watertypes
...
part_above_CCA <- plot_ordination(ps_part_above, cca_part_above,
                                  type = "sites", color="watertype")
p_CCA <- part_above_CCA + geom_point(aes(fill = watertype), size = 3, alpha = 0.7) + ggtitle("Particle-associated") + remove_grid + color_point + color_point2 +#geom_label(size= 2, aes(label = Genus)) +  # X for other watertypes
  theme(text = element_text(family = "Helvetica"), plot.title = element_text(size = 14, face = "bold", hjust = 0.5)) +
  annotate("richtext", x = Inf, y = Inf, label = "Above 200m",
           hjust = 1.1, vjust = 1.5, size = 4, color = "black",
           fontface = "bold", fill = "white") +
...
part_below_CCA <- plot_ordination(ps_part_below, cca_part_below,
                                  type = "sites", color="watertype")
p_CCA2 <- part_below_CCA + geom_point(aes(fill = watertype), size=3, alpha=0.7) + remove_grid + color_point + color_point2+#geom_label(size= 2, aes(label = Genus)) +  # X for other watertypes
  theme(text = element_text(family = "Helvetica"))+
  annotate("richtext", x = Inf, y = Inf, label = "Below 200m",
           hjust = 1.1, vjust = 1.5, size = 4, color = "black",
           fontface = "bold", fill = "white") +
...
data <- data.frame(
  x = rnorm(10),
  y = rnorm(10),
  watertype = rep(c("AASW", "AASW-WW", "WW", "WW-CDW", "CDW"), each = 2)
)

fake_plot <- ggplot(data, aes(x = x, y = y)) +
  geom_point(aes(color = watertype), size = 4) +
  color_point2 +
  shapes +
...
library(ggpubr)
ggarrange(f1,p1,f2,p2, ncol=2, nrow=2, common.legend=TRUE, legend.grob = legend_combined, legend="right", labels = c("A", "B", "C", "D"))

ggsave("final_graphics/CCA_watermass_DOC.pdf", width = 10, height = 8, dpi = 1200)
